/**
 * DISPLAY.JS
 * - Logo bumper ENTRE chaque écran
 * - Rotation / autoSwitch passent par showBumperThen()
 * - Ticker + météo conservés
 * - FIX: reset animation bumper à chaque apparition (logo visible tout le temps)
 */
...
(function(){
  ...
  const tickerFooter = document.getElementById("display-ticker");
  ...
  let config = {
    ...
    // Afficher le ticker pendant bumper
    bumperShowTicker: true,

    // ===== TICKER =====
    tickerEnabled: true,
    tickerUrl: "/rss/infos.xml", // flux par défaut
    tickerRefreshMs: 15000,
    tickerSeparator: " ",        // on laisse l'espace, le SVG est déjà dans le HTML du ticker
  ...
  function loadConfig(){
    ...
    // ticker visible/caché pendant bumper
    if (tickerFooter) {
      tickerFooter.classList.toggle("hide-during-bumper", !config.bumperShowTicker);
    }
  ...
  // ================== BUMPER ==================
  function resetBumperAnimation(){
    if(!bumperEl) return;

    // 1) remet l’état “hors show”
    bumperEl.classList.remove("show");

    // 2) force reflow pour réinitialiser toutes les animations CSS
    void bumperEl.offsetWidth;

    // 3) remet show juste après => animations repartent
    bumperEl.classList.add("show");
  }

  function showBumperThen(nextMode){
    // évite bumper si désactivé ou si déjà en cours
    ...
    resetBumperAnimation();

    // la CSS utilise --bumper-exit-delay pour caler les sorties
    bumperEl?.style.setProperty("--bumper-exit-delay", (config.bumperDurationMs/1000 - 0.8) + "s");
  ...
  // si c'est du RSS/XML → extraire les <title> d'items
  if(raw.includes("<rss") || raw.includes("<channel")){
    const titles = Array.from(raw.matchAll(/<item[\s\S]*?<title>([\s\S]*?)<\/title>/gi))
      .map(m => stripTags(m[1]))
      .filter(Boolean)
      .slice(0, 8); // limite
    if(titles.length) return titles.join(config.tickerSeparator);
  }

  // fallback texte ligne par ligne
  const lines = raw.split(/\r?\n/)
    .map(l=>stripTags(l))
    .filter(Boolean);

  if(!lines.length) return "Aucune information pour le moment";
  return lines.join(config.tickerSeparator);
}
  ...
  function applyTickerText(text){
    ...
    t1.innerHTML=text; // <-- texte déjà avec logo svg côté HTML/serveur si besoin
  ...
  function init(){
    ...
    // Premier bumper au chargement (si activé)
